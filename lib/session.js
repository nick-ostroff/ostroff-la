...
export function teslaMorningFeedToken() {
  return String(process.env.TESLA_MORNING_FEED_TOKEN || '').trim();
}

export function teslaMorningFeedAuthorized(authorizationHeader) {
  const expected = teslaMorningFeedToken();
  if (!expected) return false;
  const header = String(authorizationHeader || '');
  if (!header.startsWith('Bearer ')) return false;
  return timingSafeEqual(header.slice('Bearer '.length), expected);
