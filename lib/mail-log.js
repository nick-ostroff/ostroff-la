...
// Vercel Hobby is 64KB across all env vars. Raw mail JSON (~58KB without
// labels) already overflows once other secrets are set. Gzip + base64 the
// payload when it will not fit, and always accept that form on read.
export const ENV_VALUE_MAX = 64 * 1024;
export const ENV_GZIP_PREFIX = 'gz:';
...
export function splitLabels(raw) {
  return String(raw || '')
    .split('|')
    .map((s) => s.trim())
    .filter(Boolean);
...
export function decodeMailEnv(raw) {
  const s = String(raw || '').trim();
  if (!s) return '';
  if (s.startsWith('{') || s.startsWith('[')) return s;
  const encoded = s.startsWith(ENV_GZIP_PREFIX) ? s.slice(ENV_GZIP_PREFIX.length) : s;
  try {
    ...
    // Fall through and return the original string for JSON.parse to reject.
  }
  return s;
}

export function encodeMailEnv(json, { maxBytes = ENV_VALUE_MAX, force = false } = {}) {
  const text = typeof json === 'string' ? json : JSON.stringify(json);
  const bytes = Buffer.byteLength(text, 'utf8');
  if (!force && bytes <= maxBytes) {
    return { value: text, compressed: false, bytes, encodedBytes: bytes };
  }
  const gz = gzipSync(Buffer.from(text, 'utf8'), { level: 9 });
  const value = `${ENV_GZIP_PREFIX}${gz.toString('base64')}`;
  return {
    value,
    compressed: true,
    bytes,
    encodedBytes: Buffer.byteLength(value, 'utf8'),
  };
}
