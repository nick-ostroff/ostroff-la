...
export const NOTE_STATUSES = ['open', 'processed', 'failed', 'deduped'];
export const NOTE_TERMINAL = new Set(['processed', 'failed', 'deduped']);
export const CSV_HEADERS = [
  'id', 'account', 'date', 'from', 'subject', 'action', 'rule', 'labels',
  'cliff', 'nick', 'note', 'by', 'at',
  'status', 'processed', 'processed_at', 'processed_by', 'detail',
];
...
export function noteStatus(note) {
  if (!note || typeof note !== 'object') return 'open';
  const status = String(note.status || '').trim().toLowerCase();
  if (NOTE_STATUSES.includes(status)) return status;
  if (truthyFlag(note.processed)) return 'processed';
  return 'open';
}

export function isTerminalNoteStatus(status) {
  return NOTE_TERMINAL.has(String(status || '').trim().toLowerCase());
}

export function filterNotes(notes, status) {
  const want = String(status || '').trim().toLowerCase();
  if (!want || want === 'all') return notes || [];
  return (notes || []).filter((n) => noteStatus(n) === want);
}

export function isAckBody(body) {
  if (!body || typeof body !== 'object') return false;
  if (truthyFlag(body.processed)) return true;
  const status = String(body.status || '').trim().toLowerCase();
  return NOTE_STATUSES.includes(status);
}

export function resolveAckStatus(body) {
  if (!body || typeof body !== 'object') {
    return { error: 'ack body is required', status: 400 };
  }
  if (truthyFlag(body.processed)) return { status: 'processed' };
  const status = String(body.status || '').trim().toLowerCase();
  if (!NOTE_STATUSES.includes(status)) {
    return { error: 'status must be open, processed, failed, or deduped', status: 400 };
  }
  return { status };
}
...
export function normalizeNote(raw) {
  ...
  return {
    ...
    processed_at: String(raw.processed_at || '').trim(),
    processed_by: String(raw.processed_by || '').trim(),
    detail: String(raw.detail || '').trim().slice(0, NOTE_MAX),
  };
}
...
async function writeLocalNotes(notes) {
  ...
  const records = notes.map((n) => ({
    ...
    at: n.at,
    status: noteStatus(n),
    processed: noteStatus(n) === 'processed' ? 'true' : '',
...
function applyNoteStatus(note, { status, detail, by, at }) {
  const terminal = isTerminalNoteStatus(status);
  return normalizeNote({
    ...note,
    status,
    processed: status === 'processed',
    processed_at: terminal ? at : '',
    processed_by: terminal ? by : '',
    detail: detail != null && String(detail).trim()
      ? String(detail).trim().slice(0, NOTE_MAX)
      : (terminal ? String(note.detail || '') : ''),
  });
}

export async function ackNote(input, meta = {}) {
  const id = String(input?.id || '').trim();
  if (!id) return { error: 'note not found', status: 404 };
  const resolved = resolveAckStatus(input);
  ...
  const current = noteStatus(existing);
  if (isTerminalNoteStatus(current) && isTerminalNoteStatus(resolved.status)) {
    return { note: existing, notes, persisted: 'dedup', idempotent: true };
  }
  if (current === 'open' && resolved.status === 'open') {
    return { note: existing, notes, persisted: 'dedup', idempotent: true };
  }

  const by = meta.by || 'feed';
  const at = meta.at || new Date().toISOString();
  const updated = applyNoteStatus(existing, {
    status: resolved.status,
    detail: input.detail,
    by,
    at,
  });
  const next = mergeNoteLists(notes, [updated]);
  const saved = await saveNotes(next, meta);
  if (saved.error) return saved;
  return { note: updated, notes: next, idempotent: false, ...saved };
}
