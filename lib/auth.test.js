...
import { safeNext, signSession, readSession, COOKIE, teslaMorningFeedAuthorized } from './session.js';
...
test('middleware allows GET/POST/PATCH mail-notes with feed bearer, rejects missing token', async () => {
  ...
  try {
    ...
    const patch = await middleware(new Request('https://ostroff.la/api/mail-notes', {
      method: 'PATCH',
      headers: { authorization: 'Bearer feed-token-for-proto' },
    }));
    assert.equal(patch, undefined);

    const confirms = await middleware(new Request('https://ostroff.la/api/confirmations', {
      method: 'POST',
      headers: { authorization: 'Bearer feed-token-for-proto' },
    }));
    assert.equal(confirms, undefined);

    const noConfirm = await middleware(new Request('https://ostroff.la/api/confirmations'));
    assert.equal(noConfirm.status, 401);
...
test('teslaMorningFeedAuthorized is timing-safe and stays closed when unset', () => {
  const prev = process.env.TESLA_MORNING_FEED_TOKEN;
  try {
    delete process.env.TESLA_MORNING_FEED_TOKEN;
    assert.equal(teslaMorningFeedAuthorized('Bearer anything'), false);
    process.env.TESLA_MORNING_FEED_TOKEN = '';
    assert.equal(teslaMorningFeedAuthorized('Bearer anything'), false);
    process.env.TESLA_MORNING_FEED_TOKEN = 'otto-feed-secret';
    assert.equal(teslaMorningFeedAuthorized('Bearer otto-feed-secret'), true);
    assert.equal(teslaMorningFeedAuthorized('Bearer otto-feed-secreX'), false);
    assert.equal(teslaMorningFeedAuthorized('Basic otto-feed-secret'), false);
  } finally {
    if (prev == null) delete process.env.TESLA_MORNING_FEED_TOKEN;
    else process.env.TESLA_MORNING_FEED_TOKEN = prev;
  }
});

test('middleware allows GET/POST tesla-morning with feed bearer, rejects missing token', async () => {
  const prevSecret = process.env.ADMIN_SESSION_SECRET;
  const prevFeed = process.env.TESLA_MORNING_FEED_TOKEN;
  process.env.ADMIN_SESSION_SECRET = 'test-secret-please-rotate';
  const { default: middleware } = await import('../middleware.js');
  try {
    process.env.TESLA_MORNING_FEED_TOKEN = 'otto-feed-secret';
    const getOk = await middleware(new Request('https://ostroff.la/api/tesla-morning', {
      headers: { authorization: 'Bearer otto-feed-secret' },
    }));
    assert.equal(getOk, undefined);

    const postOk = await middleware(new Request('https://ostroff.la/api/tesla-morning', {
      method: 'POST',
      headers: { authorization: 'Bearer otto-feed-secret' },
    }));
    assert.equal(postOk, undefined);

    const denied = await middleware(new Request('https://ostroff.la/api/tesla-morning'));
    assert.equal(denied.status, 401);

    const wrong = await middleware(new Request('https://ostroff.la/api/tesla-morning', {
      method: 'POST',
      headers: { authorization: 'Bearer wrong-token-xxxxx' },
    }));
    assert.equal(wrong.status, 401);
    ...
    if (prevFeed == null) delete process.env.TESLA_MORNING_FEED_TOKEN;
    else process.env.TESLA_MORNING_FEED_TOKEN = prevFeed;
  }
});
