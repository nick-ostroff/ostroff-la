...
Plain static HTML — no framework, no build step. The only runtime dependency is `@vercel/blob` (private store for mail-log notes and the Tesla morning snapshot). Single host: ostroff.la. Existing subdomains 301 onto matching paths; do not delete tickets.ostroff.la without confirmation.

## Commands

```bash
node dev-server.js   # local dev at http://localhost:4173 (PORT env var to override)
node --test lib/auth.test.js lib/mail-log.test.js lib/mail-notes.test.js lib/tesla-morning.test.js
...
- `BLOB_READ_WRITE_TOKEN` — **required in production** so mail notes and the Tesla snapshot survive deploys. Create a **private** Blob store on the Vercel project `pixelocity/ostroff-la` (Storage → Blob → access Private) and connect it to Production. Vercel adds this token; do not invent or commit it. Without it, `POST /api/mail-notes` and `POST /api/tesla-morning` return 503.
