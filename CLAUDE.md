...
node --test lib/auth.test.js lib/mail-log.test.js lib/mail-notes.test.js lib/feed.test.js
...
- `api/feed.js` — `GET /api/feed?who=nick,peter` fetches and merges RSS feeds (`nick` → nickostroff.com/feed.xml, `peter` → peterostroff.com/feed.xml), regex-parsed, newest-first JSON, edge-cached `s-maxage=60, stale-while-revalidate=300`. Upstream `fetch` uses `cache: 'no-store'`, `Cache-Control: no-cache`, and a `_` cache-bust query so a frozen CDN body is not reused. Unknown `who` → 400; upstream failure → 502. Adding a source = add to the `PEOPLE` map.
